...
describes what users get. Pick a format with `-o`, or let letv choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
letv <command> -o table   # aligned columns for reading
letv <command> -o jsonl   # one JSON object per line, for piping
letv <command> -o json    # a single JSON array
letv <command> -o csv     # spreadsheet friendly
letv <command> -o tsv     # tab-separated
letv <command> -o url     # just the URL column
letv <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
letv <command> --fields id,title,url
```
...
```bash
letv <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
letv <command>            # a table, because this is a terminal
letv <command> | wc -l    # JSONL, because this is a pipe
```
